...
gen_data_FSTexample = function(label.size, m1, m0, s1, s0){
  # extract Y, X, G from the sample dataset
  x = FST.example$X
  y = FST.example$Y
  g = data.matrix(FST.example$G)
  # nn = 5088, which is the number of record in example
  nn = length(y)
  # Lable ids
...
dat.obj = gen_data_FSTexample(label.size = n_lab, m1, m0, s1, s0)
dat = dat.obj$my_dat
id.t = dat.obj$id.t
